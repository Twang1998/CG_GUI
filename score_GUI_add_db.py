...
class App:
    ...
    def next(self):
        ...
        if Index == len(Pic_List)-1:
            ...
            if str(Index) in list(Score.keys()):
                c.execute("UPDATE {} set Score = {} where Idx = {}".format(Name,round(self.var.get(),1),Index))
            else:
                c.execute("INSERT INTO {} (Idx,Image,Score) \
                VALUES ({}, '{}', {})".format(Name,Index,os.path.split(Pic_List[Index])[-1],round(self.var.get(),1)))
            conn.commit()
